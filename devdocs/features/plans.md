...
**Plan reading by subtask type (Flutter parity):**

| API `content_type` | Navigable when | Route | Render mode |
|---|---|---|---|
| `SOURCE_REFERENCE` | non-empty `source_text_id` | [`src/app/reader/[textId].tsx`](../../src/app/reader/[textId].tsx) | Segment reader (`POST /texts/{source_text_id}/details`), optional collapse by `segment_ids`/`pecha_segment_id` |
| `TEXT` / `INLINE_TEXT` | non-empty `content` | [`src/app/plan-text/[subtaskId].tsx`](../../src/app/plan-text/[subtaskId].tsx) | Markdown/text body with font-size controls |
| `IMAGE` | non-empty `content` URL | [`src/app/plan-text/[subtaskId].tsx`](../../src/app/plan-text/[subtaskId].tsx) | Image body (`contentMode="image"`), no font-size controls |
| other / unknown | never | dropped | Not navigable |

Route resolution is content-type-first (Flutter parity). Do **not** use `GET /texts/{id}` for reader content — that route does not exist on the backend.
