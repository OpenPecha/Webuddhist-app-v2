...
export function usePlanReadingSession(options?: { onBeforeNavigate?: () => void }) {
  ...
  const navigate = useCallback(
    async (
      direction: PlanReadingNavigateDirection,
      navigationOptions?: { autoPlay?: boolean },
    ) => {
      if (isNavigatingRef.current) return;

      onBeforeNavigateRef.current?.();
      const shouldAutoPlay = navigationOptions?.autoPlay ?? false;

      if (isPreview) {
        if (direction === 'prev') {
          if (currentIndex <= 0) return;
          if (navigateToIndex(currentIndex - 1, shouldAutoPlay)) {
